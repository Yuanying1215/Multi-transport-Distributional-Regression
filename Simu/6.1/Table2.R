# ==============================================================================
# Script: Table2.R
# Description: Comparative simulation study for Single Predictor (Section 6.1).
#              Compares RMSE of MTDR against OT (Optimal Transport) and 
#              GOT (Geodesic Optimal Transport) models.
# Output: Reproduces the RMSE values reported in Table 2.
# ==============================================================================
...
MOT <- function(Qy,Qx,bar,qSup,x,tol,inti_type,al,ar,zero,a_type){
  ...
  if (a_type == 0){
    a0 <- sum((T0nu - T1(T01))*(T0(x)-T1(T01))*w0)/
      sum(((matrix((T0(x)- T1(T01)),nrow=m))^2)*w0)
...
#OT
error2 <- function(T_res,X,Y,qSup){
  #X,Y:quantile matrix:nqSup*N
  nqSup <- length(qSup)
  N <- ncol(Y)
  Y_fit <- matrix(0,nrow = nqSup,ncol = N)
...
B <-  100
seeds <- seq(1,B,1)
for (a_true in c(0,0.3,0.5,0.7,1)){
  b <- 1 - a_true
  results <- list()
  ...
  results <- parLapply(cl,1:B,function(l){
    ...
    Y_res <- a_true*T0_true(X0r) + b*T1_true(X1r)
    #K <- sample(-100:100,N+test,replace = T)
    ...
    if (a_type==0){
      ...
      a0 <- optimize(lossfun,c(0,1),T0=T0,T1=T1,T01=T01,T0nu=T0nu,x=x,w0=w0,N=N)$minimum
    }
    if (a0 < al){a0 <- zero}
    if (a0 > ar){a0 <- 1 - zero}
    obj1_0 <- (1/N)*sum((a0*T0(x) + (1-a0)*T1(T01) - T0nu)^2 * w0)
    ...
    obj_0 <- obj1_0 + obj2_0
    cat(sprintf("Objective 0: Objective = %.12f\n", obj_0))
    result <- Main_new(T0,T1,a0,obj_0,T10,T1nu,w1,T01,T0nu,w0,x,tol=1e-9,
                       al,ar,zero,a_type)
    a_MOT <- result$a_res
    ...
    #GOT
    X1bar <- rowMeans(X1q[,1:N])
    Ybar <- rowMeans(Y[,1:N])
    alpha_res <- optimize(WassersteinLoss, c(-2, 2), Qy=Y[,1:N], y_mean=Ybar, Qx=list(X1q[,1:N]), x_mean=list(X1bar), qSup=qSup)
    gtr_pred <- sapply(1:test,function(i)
    {WassersteinPredict(alpha_res$minimum, y_mean=Ybar, qx=list(X1q[,i+N]),
                        x_mean=list(X1bar), qSup=qSup)})
    
    loss1 <- result$loss_res
    loss2 <- (1/N)*sum((T2_res(x) - T1nu)^2 * w1)
    loss3 <- alpha_res$objective
    a_GOT <- alpha_res$minimum
    MSE1 <- error(list(T0_res,T1_res),c(a_MOT,1-a_MOT),
                  list(matrix(rep(qSup,test),ncol=test),
                       X1q[,(N+1):(N+test)]),Y[,(N+1):(N+test)],qSup)
    MSE2 <- error2(T2_res,X1q[,(N+1):(N+test)],Y[,(N+1):(N+test)],qSup)
    
    MSE3 <- error3(gtr_pred,Y[,(N+1):(N+test)],qSup)
    results[[l]] <- c(MSE1$rmse,MSE2$rmse,MSE3$rmse)
  })
  stopCluster(cl)
  
  rmse1 = mean(matrix(unlist(results),nrow = 3)[1,])
  rmse1_sd = sd(matrix(unlist(results),nrow = 3)[1,])
  
  rmse2 = mean(matrix(unlist(results),nrow = 3)[2,])
  rmse2_sd = sd(matrix(unlist(results),nrow = 3)[2,])
  
  rmse3 = mean(matrix(unlist(results),nrow = 3)[3,])
  rmse3_sd = sd(matrix(unlist(results),nrow = 3)[3,])
  
  
  cat(sprintf("RMSE1,RMSE2,RMSE3: %.4f(%.4f) %.4f(%.4f) 
            %.4f(%.4f) \n", 
              rmse1,rmse1_sd,rmse2,rmse2_sd,rmse3,rmse3_sd))
}
